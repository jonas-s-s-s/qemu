/*
 * BCM2835 implementation of the ARM timer
 * =======================================
 * This is a modified version of https://github.com/qemu/qemu/pull/78 (Published on 8th Feb 2019)
 * Various modification were required as the original code would no longer compile with the newest version of QEMU.
 * For example the ptimer API has changed and now requires using a transaction oriented approach.
 *
 * Code initializing this object in bcm2835_peripherals.c was missing the sysbus_connect_irq() call, which resulted
 * in BCM283xSP804State's irq field being always NULL, this means that no IRQ was ever being emitted. Thus even the
 * original version from 2019 likely never worked as intended.
 */
...
static bcm283x_timer_state *bcm283x_timer_init(uint32_t freq)
{
    ...
    s->timer = ptimer_init(bcm283x_timer_tick, s, PTIMER_POLICY_LEGACY);
...
static void bcm283xsp804_class_init(ObjectClass *klass, void *data)
{
    ...
    device_class_set_props(k, bcm283xsp804_properties);
